...
| RISK-009 | Local developer environment may lack Ruff/MyPy even when tests pass. | Keep Ruff and MyPy in CI and developer setup; current closeout environment has both tools available, but future failures must still be recorded honestly. |
| RISK-010 | Validation evidence could be mistaken for liquidity or anomaly decisions. | Validation wording is limited to source-data quality, dispositions, and future confidence-impact inputs; it does not create forecasts, alerts, cases, or wrongdoing claims. |
| RISK-011 | Rule-based data-quality scores could be mistaken for calibrated model confidence. | Documentation and evidence label the score as deterministic rule-based data quality only; downstream confidence fusion must keep the multiplier explicit and explainable. |
