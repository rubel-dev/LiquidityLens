...
class NagadProvider(BaseProvider):
    ...
    async def get_balance(self, agent_id: str) -> ProviderBalanceResult:
        ...
        # Scenario C: simulate stale/delayed feed
        if settings.NAGAD_DELAY_SECONDS > 0:
            await asyncio.sleep(min(settings.NAGAD_DELAY_SECONDS, 5))
